...
The application is deployed to Firebase Hosting. A continuous deployment (CI/CD) pipeline is managed via GitHub Actions. Upon a push to the `main` branch, the action builds the static assets and deploys them to the global Firebase CDN, ensuring ensuring rapid load times and zero-downtime updates.